...
document.addEventListener('DOMContentLoaded', async () => {
    const SUPABASE_URL = 'https://bkgqkchtkjttmndmkmps.supabase.co';
    const SUPABASE_ANON_KEY = 'eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJpc3MiOiJzdXBhYmFzZSIsInJlZiI6ImJrZ3FrY2h0a2p0dG1uZG1rbXBzIiwicm9sZSI6ImFub24iLCJpYXQiOjE3ODQzMzcyODcsImV4cCI6MjA5OTkxMzI4N30.LjkEiWmP_maz5HyF7CudQ0MmIE12-8bTdw1dA32Yxuw';
    ...
    // --- Приветствия ---
    const greetings = [
        'О, ты снова здесь? 👀', 'С возвращением, воин! ⚔️', 'А мы тебя ждали! 😏',
        'Ты точно не робот? 🤖', 'Скучали без тебя! ❤️', 'Опять забыл пароль? 😅',
        'Ну, здравствуй! ✨', 'Ты сегодня рано! 🌅', 'Ночной дозор? 🌙',
        'Снова в деле! 💪', 'Привет, старый друг! 🤝', 'Давно не виделись! ⏰',
        'Ты как всегда вовремя! 🎯', 'Готов к приключениям? 🚀', 'С возвращением, капитан! ⚓',
    ...
    if (forgotFormFields) {
        forgotFormFields.addEventListener('submit', async (e) => {
            ...
            if (!email) { statusEl.innerHTML = '<span style="color:#ff5050">⛔ Введите email</span>'; return; }
            ...
            async function sendResetEmail(emailAddr, retryCount = 0) {
                const redirectUrl = window.location.origin + '/ru/auth/auth.html';
                const options = {};
            ...
            try {
                ...
                if (result.noRedirect) {
                    statusEl.innerHTML = '<span style="color:#44dd66">✅ Ссылка для сброса пароля отправлена на <strong>' + email + '</strong>. Проверьте почту и спам.</span><br><span style="color:#ffaa00;font-size:.78rem">⚠️ Ссылка без авто-редиректа — скопируйте её из письма вручную.</span>';
                } else {
                    statusEl.innerHTML = '<span style="color:#44dd66">✅ Ссылка для сброса пароля отправлена на <strong>' + email + '</strong>. Проверьте почту и спам.</span>';
                }
    ...
    if (resetPwdInput) {
        resetPwdInput.addEventListener('input', function() {
            ...
            if (!pwd) { text.textContent = 'Слабый'; return; }
            if (score <= 1) { bar.classList.add('weak'); text.textContent = 'Слабый'; }
            else if (score <= 3) { bar.classList.add('medium'); text.textContent = 'Средний'; }
            else if (score <= 5) { bar.classList.add('strong'); text.textContent = 'Сильный'; }
            else { bar.classList.add('very-strong'); text.textContent = 'Очень сильный'; }
        });
    }
    ...
    if (resetFormFields) {
        resetFormFields.addEventListener('submit', async (e) => {
            ...
            if (password.length < 8) { statusEl.innerHTML = '<span style="color:#ff5050">⛔ Пароль минимум 8 символов</span>'; return; }
            if (password !== confirm) { statusEl.innerHTML = '<span style="color:#ff5050">⛔ Пароли не совпадают</span>'; return; }
            ...
            try {
                ...
                if (error) throw error;
                statusEl.innerHTML = '<span style="color:#44dd66">✅ Пароль успешно изменён! Перенаправление на вход...</span>';
                setTimeout(async () => {
    ...
    modalConfirm.addEventListener('click', async function() {
        ...
        if (code.length < 6) {
            modalStatus.textContent = '⛔ Введите все 6 цифр';
        ...
        if (code === verificationCode) {
            ...
            codeAttempts++;
            if (codeAttempts >= 3) {
                modalStatus.textContent = '❌ Слишком много попыток. Запросите новый код';
                modalStatus.className = 'modal-status error';
                this.disabled = true;
                return;
            }
            modalStatus.textContent = `❌ Неверный код. Осталось попыток: ${3 - codeAttempts}`;
            modalStatus.className = 'modal-status error';
            this.disabled = false;
    ...
    modalResend.addEventListener('click', async function() {
        if (!canResend) {
            modalStatus.textContent = '⏳ Подождите 30 секунд перед повторной отправкой';
            modalStatus.className = 'modal-status error';
            return;
        }
        this.disabled = true;
    ...
    if (regFormFields) {
        regFormFields.addEventListener('submit', async (e) => {
            ...
            if (age < 13) {
                showNotification('⛔ Регистрация только для пользователей от 13 лет', 'error');
                return;
            }
            if (username.length < 3) {
                showNotification('⛔ Username минимум 3 символа', 'error');
                return;
            }
    ...
    mfaModalConfirm.addEventListener('click', async function() {
        ...
        if (code.length < 6) {
            mfaModalStatus.textContent = '⛔ Введите все 6 цифр';
    ...
    mfaModalCancel.addEventListener('click', async function() {
        hideMfaModal();
        await supabase.auth.signOut();
        showNotification('❌ Вход отменён', 'error');
    ...
    document.getElementById('login-form-fields').addEventListener('submit', async (e) => {
        ...
        try {
            ...
            if (!identifierVal.includes('@')) {
                ...
                if (!email) {
                    identifier.classList.add('input-error');
                    showNotification('❌ У пользователя не указан email', 'error');
                    submitBtn.disabled = false;
                    setTimeout(()=>identifier.classList.remove('input-error'),2000);
                    return;
                }
            ...
            if (error) {
                ...
                if (msg.includes('Invalid login credentials') || msg.includes('invalid_credentials')) {
                    ...
                    showNotification('❌ Слишком много попыток. Подождите.', 'error');
                } else {
            ...
            try {
                ...
                if (banData) {
                    if (banData.banned_until && new Date(banData.banned_until) < new Date()) {
                        ...
                        await supabase.auth.signOut();
                        const reason = banData.reason || 'Нарушение правил';
                        ...
                        showNotification('🔒 Аккаунт заблокирован. Причина: ' + reason + ' (до: ' + until + ')', 'error', 8000);
                        submitBtn.disabled = false;
                        return;
                    }
                }
            ...
            if (isMfaEnabled && loadedMfaSecret) {
                if (!window.crypto || !window.crypto.subtle) {
                    await supabase.auth.signOut();
                    showNotification('❌ Web Crypto API недоступен. Используйте HTTPS.', 'error');
                    submitBtn.disabled = false;
                    return;
                }
        ...
        btn.addEventListener('click', async () => {
            ...
            try {
                ...
                const { error } = await supabase.auth.signInWithOAuth({
                    provider,
                    options: { redirectTo: window.location.origin + '/ru/auth/auth.html' }
                });
                if (error) throw error;
            } catch (err) {
                showNotification(`❌ ${err.message}`, 'error');
