...
if(!isAdmin){
  document.body.innerHTML=`<div style="display:flex;align-items:center;justify-content:center;height:100vh;color:#ff5050;font-size:1.2rem;font-family:sans-serif"><div style="text-align:center"><i class="fas fa-lock" style="font-size:3rem;margin-bottom:16px;display:block"></i>Доступ запрещён<br><small style="color:#888">Роль: ${ud?.role||'не найдена'}</small><br><a href="../main/index.html" style="color:#ff8800;margin-top:12px;display:inline-block">← На главную</a></div></div>`;
  return;
}
...
try{
  ...
  if(banData){
    if(banData.banned_until && new Date(banData.banned_until)<new Date()){
      ...
      document.body.innerHTML='<div style="display:flex;align-items:center;justify-content:center;height:100vh;color:#ff5050;font-size:1.2rem;font-family:sans-serif"><div style="text-align:center"><div style="font-size:4rem;margin-bottom:16px">🔒</div>Аккаунт заблокирован<br><a href="../auth/auth.html" style="color:#ff8800;margin-top:12px;display:inline-block">Выйти</a></div></div>';return;
    }
  }
}catch(e){}
...
function showUserDetail(u){
  ...
  <div class="user-detail-body">
    ...
    <div class="user-detail-grid">
      ...
      <div class="ud-field"><i class="fas fa-birthday-cake"></i><span class="ud-label">День рождения</span><span class="ud-value">${u.birthday||'Не указана'}</span></div>
    ...
    <div class="user-detail-section"><h4><i class="fas fa-desktop"></i> Сессии</h4><div id="user-sessions-list" style="max-height:180px;overflow-y:auto">Загрузка...</div></div>
    <div class="user-detail-section"><h4><i class="fas fa-history"></i> Последние действия</h4><div id="user-audit-list" style="max-height:180px;overflow-y:auto">Загрузка...</div></div>
    <div class="user-detail-section"><h4><i class="fas fa-link"></i> Ссылки профиля</h4><div id="user-links-list">Загрузка...</div></div>
    <div class="user-detail-actions">
      ${isBanned?`<button class="btn-primary" id="ud-unban-btn" data-uid="${u.id}"><i class="fas fa-unlock"></i> Разбанить</button>`:`<button class="btn-danger" id="ud-ban-btn" data-uid="${u.id}" data-username="${u.username||''}"><i class="fas fa-ban"></i> Забанить</button>`}
    ...
    ${isBanned?`<div class="user-detail-ban-info"><i class="fas fa-ban"></i> Забанен до: <strong>${u.banned_until==='permanent'?'Навсегда':new Date(u.banned_until).toLocaleString('ru-RU')}</strong><br>Причина: ${u.ban_reason||'Не указана'}</div>`:''}
  </div>`;
  ...
  (async()=>{
    ...
    try{
      ...
      if(!sessions||!sessions.length){c.innerHTML='<div style="color:var(--text-3);font-size:.82rem;padding:8px">Нет сессий</div>';return}
      c.innerHTML='';
      sessions.forEach(s=>{
        ...
        killBtn.addEventListener('click',async()=>{
          try{const{error}=await sb.from('user_sessions').delete().eq('id',s.id);if(error)throw error;toast('Сессия удалена','success');showUserDetail(u)}catch(e){toast('Ошибка: '+e.message,'error',6000)}
        });
  ...
  (async()=>{
    ...
    try{
      ...
      if(error)throw error;
      if(!logs||!logs.length){c.innerHTML='<div style="color:var(--text-3);font-size:.82rem;padding:8px">Нет записей</div>';return}
      c.innerHTML='';
  ...
  (async()=>{
    ...
    try{
      ...
      if(error)throw error;
      if(!links||!links.length){c.innerHTML='<div style="color:var(--text-3);font-size:.82rem;padding:8px">Нет ссылок</div>';return}
      c.innerHTML='';
  ...
  document.getElementById('ud-ban-btn')?.addEventListener('click',async function(){
    ...
    try{
      ...
      const{data:banData,error:banErr}=await sb.from('user_bans').upsert({user_id:uid,reason:reason||'Нарушение правил',banned_until:bannedUntil,banned_by:user.id,created_at:new Date().toISOString()},{onConflict:'user_id'});
      ...
      try{const{error:nErr}=await sb.from('user_notifications').insert({user_id:uid,type:'system',title:'Аккаунт заблокирован',message:'Причина: '+(reason||'Нарушение правил')});if(nErr)console.warn('Notif:',nErr.message)}catch(e){}
      ...
      toast('Пользователь забанен','success');overlay.remove();loadUsers();
    }catch(e){toast('Ошибка: '+e.message,'error',6000)}
  });

  // UNBAN BUTTON
  document.getElementById('ud-unban-btn')?.addEventListener('click',async function(){
    const uid=this.dataset.uid;if(!confirm('Разбанить пользователя?'))return;
    try{
      ...
      toast('Пользователь разбанен','success');overlay.remove();loadUsers();
    }catch(e){toast('Ошибка: '+e.message,'error',6000)}
  });
  ...
  document.getElementById('ud-role-btn')?.addEventListener('click',async function(){
    ...
    try{
      ...
      toast('Роль обновлена','success');overlay.remove();loadUsers();
    }catch(e){toast('Ошибка: '+e.message,'error',6000)}
  });
  ...
  document.getElementById('ud-kill-sessions-btn')?.addEventListener('click',async function(){
    ...
    try{
      ...
      if(r.error){toast('Ошибка: '+r.error.message,'error',6000);return}
      toast('Завершено сессий: '+(r.data||0),'success');
    }catch(e){toast('Ошибка: '+e.message,'error',6000)}
  });
  ...
  document.getElementById('ud-reset-pw-btn')?.addEventListener('click',async function(){
    ...
    try{const{error}=await sb.auth.resetPasswordForEmail(email,{redirectTo:window.location.origin+'/ru/auth/auth.html'});if(error)throw error;toast('Письмо отправлено','success')}catch(e){toast('Ошибка: '+e.message,'error',6000)}
  });
}
...
try{
  ...
  if(!filtered||!filtered.length){tbody.innerHTML='<tr><td colspan="8" style="text-align:center;color:var(--text-3)">Нет пользователей</td></tr>';return}
...
try{
  ...
  if(failed>0){toast(`❌ Ошибка: ${lastErr}`,'error',8000)}else{toast('✅ Настройки сервера сохранены','success')}
}catch(e){toast('❌ Ошибка: '+e.message,'error',6000)}
...
if(!newsItems.length){list.innerHTML='<div style="text-align:center;color:var(--text-3);padding:20px">Нет новостей</div>';return}
newsItems.forEach(n=>{
...
const titleDiv=document.createElement('div');titleDiv.className='news-admin-title';titleDiv.textContent=n.title||'Без заголовка';
...
delBtn.addEventListener('click',async()=>{
  ...
  try{const{error}=await sb.from('news').delete().eq('id',n.id);if(error)throw error;toast('Удалено','success');loadNewsAdmin()}catch(e){toast('Ошибка: '+e.message,'error',6000)}
});
...
if(!title||!content){toast('Заполните заголовок и текст','error');return}
try{let r;
...
toast(editId?'✅ Обновлено':'✅ Добавлено','success');el('news-form-card').style.display='none';loadNewsAdmin();
}catch(e){toast('❌ Ошибка: '+e.message,'error',6000)}
});
...
try{
  ...
  if(!filtered.length){tbody.innerHTML='<tr><td colspan="5" style="text-align:center;color:var(--text-3)">Нет записей</td></tr>';return}
...
try{
  ...
  if(error){list.innerHTML=`<div style="color:var(--red)">Ошибка: ${error.message}</div>`;return}
  if(!bans||!bans.length){list.innerHTML='<div style="text-align:center;color:var(--text-3);padding:20px">Нет забаненных</div>';return}
  list.innerHTML='';
  ...
  bans.forEach(b=>{
    ...
    const tdReason=document.createElement('td');tdReason.textContent=b.reason||'Не указана';
    const tdUntil=document.createElement('td');tdUntil.style.fontSize='.78rem';tdUntil.textContent=!b.banned_until?'Навсегда':new Date(b.banned_until).toLocaleString('ru-RU');
    ...
    const tdActions=document.createElement('td');tdActions.className='action-btns';
    const unbanBtn=document.createElement('button');unbanBtn.className='action-btn';unbanBtn.innerHTML='<i class="fas fa-unlock"></i> Разбанить';
    unbanBtn.addEventListener('click',async()=>{
      if(!confirm('Разбанить '+(userMap[b.user_id]||'пользователя')+'?'))return;
      try{
...
if(!userId){toast('Укажите email или ID','error');return}
try{
  let targetId=userId;
  if(!userId.includes('-')&&!userId.includes('@')){
    ...
    if(found&&found.length)targetId=found[0].id;else{toast('Пользователь не найден','error');return}
  }else if(userId.includes('@')){
    const{data:found,error}=await sb.from('users').select('id').eq('email',userId).limit(1);
    if(error){toast('Ошибка поиска: '+error.message,'error',6000);return}
    if(found&&found.length)targetId=found[0].id;else{toast('Email не найден','error');return}
  }
  const{error:banErr}=await sb.from('user_bans').upsert({user_id:targetId,reason:reason||'Нарушение правил',banned_by:user.id,created_at:new Date().toISOString()},{onConflict:'user_id'});
  if(banErr){toast('Ошибка бана: '+banErr.message,'error',6000);return}
  ...
  try{const{error:nErr}=await sb.from('user_notifications').insert({user_id:targetId,type:'system',title:'Аккаунт заблокирован',message:'Причина: '+(reason||'Нарушение')});if(nErr)console.warn('Notif:',nErr.message)}catch(e){}
  toast('Пользователь забанен','success');el('ban-user-id').value='';el('ban-reason').value='';loadBans();
}catch(e){toast('Ошибка: '+e.message,'error',6000)}
});
...
window.adminBroadcast=async function(){
  ...
  try{
    ...
    if(!users?.length){toast('Нет пользователей','info');return}
    let sent=0;
    ...
    toast('Отправлено '+sent+'/'+users.length,'success');
  }catch(e){toast('Ошибка: '+e.message,'error',6000)}
};
...
window.adminPushAll=async function(){
  ...
  try{
    ...
    if(error){toast('Ошибка: '+error.message,'error',6000);return}
    if(!subs?.length){toast('Нет подписчиков','info');return}
    ...
    toast('Push отправлен '+uniqueIds.length+' пользователям','success');
  }catch(e){toast('Ошибка: '+e.message,'error',6000)}
};
